...
const getChatsList = async ({
	page,
	size,
}: InfiniteParams): Promise<ResponseChatRoom> => {
	const { data } = await axiosInstance.get(
		`${API_URL.GET_CHAT_ROOMS}?page=${page}&size=${size}`,
	);
...
const getChatContent = async ({
	...
	page = 1,
	size = 1,
}: TGetChatContent): Promise<ResponseChatList> => {
	const { data } = await axiosInstance.get(
		`${API_URL.GET_CHAT_CONTENT}/${chatRoomId}?page=${page}&size=${size}`,
	);
